...
class ESPLoader(object):
    ...
    def __init__(self, port=DEFAULT_PORT, baud=ESP_ROM_BAUD, trace_enabled=False, boardtype="esp32dev"):
        ...
        print("Board type {}".format(boardtype))
    ...
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, connect_mode='default_reset', trace_enabled=False, boardtype="esp32dev"):
        ...
        """
        detect_port = ESPLoader(port, baud, trace_enabled=trace_enabled, boardtype=boardtype)
    ...
    def resetToBootloader(self, esp32r0_delay=False):
        ...
        if self.boardtype == "esp32dev":
            ...
            self._setDTR(False)  # IO0=0
            self._setRTS(False)  # EN=LOW, chip in reset
            time.sleep(0.1)
            if esp32r0_delay:
                # Some chips are more likely to trigger the esp32r0
                # watchdog reset silicon bug if they're held with EN=LOW
    ...
    def hard_reset(self):
        if self.boardtype == "esp32dev":
            ...
            self._setRTS(0) # IO0 = 0
            self._setDTR(0) # EN = 0
            time.sleep(0.1)
            self._setDTR(1) # EN = 1
            self._setRTS(1) # IO0 = 1
    ...
    def _connect_attempt(self, mode='default_reset', esp32r0_delay=False):
        ...
        if mode != 'no_reset':
            self.resetToBootloader(esp32r0_delay)
...
def main():
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        for each_port in reversed(ser_list):
            ...
            try:
                if args.chip == 'auto':
                    esp = ESPLoader.detect_chip(each_port, initial_baud, args.before, args.trace, args.board)
                else:
                    chip_class = {
                        'esp8266': ESP8266ROM,
                        'esp32': ESP32ROM,
                    }[args.chip]
                    esp = chip_class(each_port, initial_baud, args.trace, args.board)
